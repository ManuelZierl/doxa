...
def test_rule_head_var_arg_from_doxa() -> None:
    arg = RuleHeadVarArg.from_doxa("X")
...
def test_rule_head_literal_arg_from_doxa_string() -> None:
    arg = RuleHeadLiteralArg.from_doxa('"hello world"')
...
def test_rule_head_literal_arg_from_doxa_int() -> None:
    arg = RuleHeadLiteralArg.from_doxa("42")
...
def test_rule_head_literal_arg_from_doxa_float() -> None:
    arg = RuleHeadLiteralArg.from_doxa("3.14")
...
def test_rule_head_literal_arg_rejects_invalid_literal() -> None:
    with pytest.raises(ValueError, match="Invalid rule head literal argument"):
        RuleHeadLiteralArg.from_doxa("not_a_literal")
...
def test_rule_head_arg_dispatch_literal_string() -> None:
    arg = rule_head_arg_from_doxa('"abc"')
...
def test_rule_head_arg_dispatch_literal_int() -> None:
    arg = rule_head_arg_from_doxa("10")
    assert isinstance(arg, RuleHeadLiteralArg)
    assert arg.lit_type == LiteralType.int
    assert arg.value == 10
...
def test_rule_goal_var_arg_from_doxa() -> None:
    arg = RuleGoalVarArg.from_doxa("X")
...
def test_rule_goal_literal_arg_from_doxa_string() -> None:
    arg = RuleGoalLiteralArg.from_doxa('"hello world"')
...
def test_rule_goal_literal_arg_from_doxa_int() -> None:
    arg = RuleGoalLiteralArg.from_doxa("42")
...
def test_rule_goal_literal_arg_from_doxa_float() -> None:
    arg = RuleGoalLiteralArg.from_doxa("3.14")
...
def test_rule_goal_literal_arg_rejects_invalid_literal() -> None:
    with pytest.raises(ValueError, match="Invalid rule goal literal argument"):
        RuleGoalLiteralArg.from_doxa("not_a_literal")
...
def test_rule_goal_arg_dispatch_literal_string() -> None:
    arg = rule_goal_arg_from_doxa('"abc"')
...
def test_rule_goal_arg_dispatch_literal_int() -> None:
    arg = rule_goal_arg_from_doxa("10")
    assert isinstance(arg, RuleGoalLiteralArg)
    assert arg.lit_type == LiteralType.int
    assert arg.value == 10
...
def test_rule_atom_goal_rejects_wrong_arity() -> None:
    with pytest.raises(ValidationError):
        RuleAtomGoal(
            ...
            pred_arity=2,
            negated=False,
            goal_args=[RuleGoalVarArg.from_doxa("X")],
        )


def test_rule_builtin_goal_from_doxa() -> None:
    goal = RuleBuiltinGoal.from_doxa("eq(X, Y)")
...
def test_rule_goal_dispatch_rejects_negated_builtin() -> None:
    with pytest.raises(ValueError, match="Builtin goals cannot be negated"):
        rule_goal_from_doxa("not eq(X, Y)")


def test_rule_from_doxa_simple() -> None:
...
def test_rule_from_doxa_with_builtin_goal() -> None:
    rule = Rule.from_doxa("ok(X) :- risk_score(X, S), geq(S, 0)")
...
def test_rule_from_doxa_with_head_literal() -> None:
    rule = Rule.from_doxa('answer("yes") :- ready(true)')
...
def test_rule_from_doxa_rejects_non_string() -> None:
    with pytest.raises(TypeError, match="must be a string"):
        Rule.from_doxa(None)  # type: ignore[arg-type]


def test_rule_from_doxa_rejects_empty_input() -> None:
...
def test_rule_from_doxa_rejects_invalid_syntax(inp: str) -> None:
...
def test_rule_validate_head_arity_rejects_wrong_arg_count() -> None:
    with pytest.raises(ValidationError):
        Rule(
            kind=BaseKind.rule,
            created_at=Rule.from_doxa("ancestor(X, Y) :- parent(X, Y)").created_at,
            head_pred_name="ancestor",
            head_pred_arity=2,
            head_args=[RuleHeadVarArg.from_doxa("X")],
            goals=[RuleAtomGoal.from_doxa("parent(X, Y)")],
        )


def test_rule_validate_goal_indices_rejects_non_contiguous_indices() -> None:
    g0 = RuleAtomGoal.from_doxa("parent(X, Y)")
    g1 = RuleAtomGoal.from_doxa("person(X)")
    ...
    with pytest.raises(ValidationError):
        Rule(
            ...
            head_pred_arity=2,
            head_args=[
                RuleHeadVarArg.from_doxa("X").model_copy(update={"pos": 0}),
                RuleHeadVarArg.from_doxa("Y").model_copy(update={"pos": 1}),
            ],
            goals=[
...
def test_rule_round_trip_simple() -> None:
    original = Rule.from_doxa("ancestor(X, Y) :- parent(X, Y)")
    reparsed = Rule.from_doxa(original.to_doxa())
...
def test_rule_string_literal_round_trip() -> None:
    original = Rule.from_doxa('label_out(X, "hello") :- label_in(X, "hello")')
    reparsed = Rule.from_doxa(original.to_doxa())
...
def test_rule_bool_literal_round_trip() -> None:
    original = Rule.from_doxa("active_out(X, true) :- active_in(X, true)")
    reparsed = Rule.from_doxa(original.to_doxa())
