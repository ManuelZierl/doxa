...
fn test_predicate_registration_and_symbol_store() {
    ...
    let profile = store
        ...
        .register_predicate(
            ...
            EvidenceMode::None,
            vec![IndexSpec::new("by_first", vec![0]), IndexSpec::new("by_second", vec![1])],
        )
        .unwrap();
    ...
    assert_eq!(store.symbol_store.get_text(sym_a).unwrap().unwrap(), "alice");
...
fn test_upsert_maximum() {
    ...
    let atom_key = AtomKey { pred_id: profile.pred_id, args: vec![s1, s2] };
    // insert first contribution (b=0.4, d=0.0)
    let state = store
        .upsert_atom(&atom_key, &Contribution { b: 0.4, d: 0.0, evidence_id: None })
        .unwrap();
    assert!((state.b - 0.4).abs() < 1e-6);
    // insert second contribution (b=0.6, d=0.1)
    let state = store
        .upsert_atom(&atom_key, &Contribution { b: 0.6, d: 0.1, evidence_id: None })
        .unwrap();
...
fn test_upsert_noisy_or_with_retract() {
    ...
    let atom_key = AtomKey { pred_id: profile.pred_id, args: vec![s1] };
    // insert two contributions from distinct sources
    let c1 = Contribution { b: 0.5, d: 0.0, evidence_id: Some(b"src1".to_vec()) };
    let c2 = Contribution { b: 0.3, d: 0.0, evidence_id: Some(b"src2".to_vec()) };
...
fn test_capped_sum() {
    ...
    let c1 = Contribution { b: 0.4, d: 0.0, evidence_id: Some(b"e1".to_vec()) };
    let c2 = Contribution { b: 0.5, d: 0.0, evidence_id: Some(b"e2".to_vec()) };
    let c3 = Contribution { b: 0.3, d: 0.0, evidence_id: Some(b"e3".to_vec()) };
...
fn test_upsert_existing_evidence_updates() {
    ...
    // Insert contribution
    let c1 = Contribution { b: 0.3, d: 0.0, evidence_id: Some(b"ev1".to_vec()) };
    store.upsert_atom(&ak, &c1).unwrap();
    // Update same evidence ID with different value
    let c1_updated = Contribution { b: 0.8, d: 0.0, evidence_id: Some(b"ev1".to_vec()) };
...
fn test_invalid_arity_error() {
    ...
    let result = store.upsert_atom(&ak, &Contribution { b: 0.5, d: 0.0, evidence_id: None });
    match result {
        Err(StoreError::InvalidArity { expected: 2, found: 1 }) => {}
...
fn test_persistence_roundtrip() {
    ...
    // Open, insert data, flush, and drop
    {
        ...
        let ak = AtomKey::new(profile.pred_id, vec![s1]);
        store
            .upsert_atom(&ak, &Contribution { b: 0.9, d: 0.1, evidence_id: None })
            .unwrap();
...
fn test_atom_state_approx_eq() {
    let a = AtomState { b: 0.1 + 0.2, d: 0.0 };
